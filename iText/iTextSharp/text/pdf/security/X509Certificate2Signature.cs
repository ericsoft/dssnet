...
namespace iTextSharp.text.pdf.security
{
    ...
    public class X509Certificate2Signature : IExternalSignature
    {
        ...
        {
            if (certificate.PrivateKey is RSACryptoServiceProvider provider)
            {
                // Force rsa all PrivateKey properties 
                //Including  Private Parameters  
                //Instead of cast  that does not always work
                //RSACryptoServiceProvider rsa = certificate.PrivateKey; new   RSACryptoServiceProvider() ;
                //rsa.FromXmlString(certificate.PrivateKey.ToXmlString(true)) ;      
                return provider.SignData(message, hashAlgorithm);
            }
            else
            {
